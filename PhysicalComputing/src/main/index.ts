...
const createWindow = (): void => {
  ...
  board.on("ready", () => {
    let redLed = new five.Led(3);
    let blueLed=new five.Led(11);
    // // led.blink(1000);
    ipcMain.handle('write:RedLEDStatus', (event: any ,value: 1|0) => {
      console.log(value)
      if (value === 0) {
        redLed.off()
      } else {
    ...
    ipcMain.handle('write:BlueLEDStatus', (event: any ,value: 1|0) => {
      console.log(value)
      if (value === 0) {
        blueLed.off()
      } else {
    ...
    let potentiometerL = new five.Sensor({
      pin: "A0",
      frequency: 250,// 1/4 of a second
      threshold: 5 
    });
    potentiometerL.on("change", function () {
      console.log(this.value / 1023.0)
      mainWindow.webContents.send('update-position-x-l', (this.value/1023.0)*2-1)
    });
    let potentiometerR = new five.Sensor({
      pin: "A1",
      frequency: 250,// 1/4 of a second
      threshold: 5 
    });
    potentiometerR.on("change", function () {
      console.log(this.value / 1023.0)
      mainWindow.webContents.send('update-position-x-r', (this.value/1023.0)*2-1)
    });
  })
